...
Expected: sticky top bar with "ZEROCALC" wordmark (CALC in yellow) and 4 nav links visible on the dark indigo background; clicking each link navigates without a 404 (pages are still placeholders until later tasks — a default Next.js 404 for `/drop-finder` etc. at this point is expected and fine).
